...
class _TeacherDisputeScreenState extends ConsumerState<TeacherDisputeScreen> {
  ...
  Future<void> _attachEvidence() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text(l.disputeUploadErr(e.toString()))));
    } finally {
  ...
  Future<void> _submitResponse() async {
    ...
    if (_response.text.trim().isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text(l.disputeErrEmptyResponse)));
      return;
    }
    setState(() => _submitting = true);
    try {
      final dispute = ref.read(_disputeProvider(widget.disputeId)).value;
      final disputeRowId = dispute?['id'] as String?;
      if (disputeRowId == null) throw Exception('لم يُعثر على النزاع');
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l.disputeResponseSent),
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text(l.disputeSubmitErr(e.toString()))));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          _AppBar(disputeId: widget.disputeId, l: l),
          Expanded(
            child: disputeAsync.when(
              loading: () => const Center(child: CircularProgressIndicator(color: AppColors.primary)),
              error: (e, _) => Center(child: Text(l.disputeLoadErr(e.toString()))),
              data: (dispute) => SingleChildScrollView(
                padding: const EdgeInsets.all(22),
                child: Column(
                  children: [
                    _DisputeHero(status: dispute?['status'] as String? ?? 'open', l: l),
                    const SizedBox(height: 16),
                    _DisputeDetails(dispute: dispute, l: l),
                    const SizedBox(height: 16),
                    if (dispute?['student_statement'] != null) ...[
                      _StudentComplaint(
                          statement: dispute!['student_statement'] as String, l: l),
                      const SizedBox(height: 16),
                    ],
                    if (dispute?['teacher_statement'] == null) ...[
                      _ResponseField(controller: _response, l: l),
                      const SizedBox(height: 16),
                      if (_evidenceUrls.isNotEmpty)
                        _EvidenceList(urls: _evidenceUrls, l: l),
                    ] else
                      _AlreadyResponded(
                          statement: dispute!['teacher_statement'] as String, l: l),
                    const SizedBox(height: 16),
                    _AdminNote(l: l),
...
class _AppBar extends StatelessWidget {
  final String disputeId;
  final dynamic l;
  const _AppBar({required this.disputeId, required this.l});
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text('${l.disputePrefix}${disputeId.substring(0, 8).toUpperCase()}',
...
class _DisputeHero extends StatelessWidget {
  final String status;
  final dynamic l;
  const _DisputeHero({required this.status, required this.l});
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Text(
              isOpen ? l.disputeStatusOpen : l.disputeStatusResolved,
              style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Colors.white),
            ),
          ),
          const SizedBox(height: 11),
          Text(
            isOpen ? l.disputeOpenedBody : l.disputeResolvedBody,
            style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textPrimary),
          ),
          if (isOpen) ...[
            const SizedBox(height: 5),
            Text(
              l.disputeDeadlineNote,
              style: const TextStyle(fontSize: 12.5, color: Color(0xFF7A4A4A), height: 1.6),
            ),
          ],
        ],
      ),
...
class _DisputeDetails extends StatelessWidget {
  ...
  final dynamic l;
  const _DisputeDetails({required this.dispute, required this.l});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          _Row(label: l.disputeReasonLabel, value: reason),
          const SizedBox(height: 9),
          _Row(label: l.disputeSubjectLabel, value: subject),
          const SizedBox(height: 9),
          _Row(label: l.disputeStudentLabel, value: studentName),
          const SizedBox(height: 9),
          _Row(label: l.disputeFrozenAmtLabel,
              value: '$frozenAmt ${l.dashOugiya}',
...
class _StudentComplaint extends StatelessWidget {
  ...
  final dynamic l;
  const _StudentComplaint({required this.statement, required this.l});
...
class _AlreadyResponded extends StatelessWidget {
  final String statement;
  final dynamic l;
  const _AlreadyResponded({required this.statement, required this.l});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(l.disputeSentResponseTitle,
            style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFF15805F))),
          const SizedBox(height: 7),
          Text(statement,
            style: const TextStyle(fontSize: 12.5, color: AppColors.textSecondary, height: 1.7)),
        ],
...
class _ResponseField extends StatelessWidget {
  ...
  final dynamic l;
  const _ResponseField({required this.controller, required this.l});
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: l.disputeResponseHint,
...
class _EvidenceList extends StatelessWidget {
  ...
  final dynamic l;
  const _EvidenceList({required this.urls, required this.l});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(l.disputeEvidenceCount(urls.length),
            style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8, runSpacing: 8,
            children: urls.map((_) => Container(
              ...
              child: Row(
                ...
                children: [
                  const Icon(Icons.attach_file_rounded, size: 13, color: Color(0xFF15805F)),
                  const SizedBox(width: 4),
                  Text(l.disputeImageLabel,
                      style: const TextStyle(fontSize: 11, color: Color(0xFF15805F))),
                ],
              ),
            )).toList(),
          ),
...
class _AdminNote extends StatelessWidget {
  final dynamic l;
  const _AdminNote({required this.l});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.shield_outlined, color: Color(0xFF7B61FF), size: 20),
          const SizedBox(width: 10),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(l.disputeAdminDecisionLabel,
                style: const TextStyle(fontSize: 11, color: Color(0xFF8A78D6))),
              Text(l.disputeAdminDecisionValue,
...
class _BottomActions extends StatelessWidget {
  ...
  const _BottomActions({
    ...
    required this.onSubmit, required this.onAttach, required this.l,
  });
