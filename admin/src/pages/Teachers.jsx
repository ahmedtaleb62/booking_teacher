...
export default function Teachers() {
  ...
  async function approve(id) {
    ...
    const { error } = await supabase
      .from('teacher_profiles').update({ is_approved: true }).eq('id', id)
    if (error) {
      setActionId(null)
      toast('خطأ في اعتماد الأستاذ: ' + error.message, 'error')
      return
    }
    // Notify the teacher
    ...
    setActionId(null)
    setModal(null)
    toast('تم اعتماد الأستاذ بنجاح', 'success')
    await loadData()
  }
  ...
  async function reject(id) {
    ...
    const { error: delErr } = await supabase
      .from('teacher_profiles').delete().eq('id', id)
    if (delErr) {
      setActionId(null)
      toast('خطأ في رفض الطلب: ' + delErr.message, 'error')
      return
    }
    ...
    if (updErr) {
      setActionId(null)
      toast('خطأ في تحديث حالة الحساب: ' + updErr.message, 'error')
      return
    }
    // Notify the teacher
    await supabase.from('notifications').insert({
      user_id: id,
      title:   'اعتذرنا عن طلبك',
      body:    'لا يمكن قبول طلبك في الوقت الحالي. يرجى التواصل معنا للمزيد.',
        ...
        {apps.map(t => (
          <div key={t.id} className="card" style={{ cursor: 'pointer' }} onClick={() => setModal(t)}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text4)', marginTop: 8 }}>
              تقدّم {timeAgo(t.created_at || t.profiles?.created_at)}
            </div>
          </div>
        ))}
      </div>
